...
enum Commands {
    ...
    Render {
        ...
        /// Theme name (blueprint, sumi, watercolor, chalk, none)
    ...
    match cli.command {
        ...
        Commands::Transform {
            ...
            let theme_enum = match theme.as_str() {
                ...
                "none" => Theme::None,
                _ => {
                    eprintln!("Error: theme `{theme}` is not implemented yet. Currently only `blueprint`, `sumi`, `watercolor`, `chalk`, and `none` are supported.");
                    std::process::exit(1);
                }
            };
